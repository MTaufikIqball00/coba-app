...
const MiniBar = ({ value, percentage, color }: { value: string | number, percentage: number, color: string }) => (
  <div className="flex items-center gap-3">
    ...
    <div className="w-full bg-gray-200 rounded-full h-2.5">
      <div className={`${color} h-2.5 rounded-full`} style={{ width: `${Math.min(100, percentage)}%` }}></div>
    </div>
  </div>
);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          <div className="overflow-x-auto">
            {loading ? <div className="p-10 text-center">Loading reports...</div> : (
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {reports.length === 0 ? (
                    <tr><td colSpan={4} className="text-center p-4">Belum ada data akademik.</td></tr>
                  ) : reports.map((report) => (
                    <tr key={report.class}>
        ...
        {!loading && reports.some(r => r.subjectBreakdown.length > 0) && (
          <div className="mt-8">
